...
class MonthView @JvmOverloads constructor(context: Context,attributeSet: AttributeSet):LinearLayout(context,attributeSet) {
    ...
    var listner:AdapterClickListener?=null
    ...
    private fun setWeekView() {
        ...
        var days: ArrayList<LocalDate?> = daysInMonthArray()
        ...
        val layoutManager: RecyclerView.LayoutManager = GridLayoutManager(context.applicationContext, 7,LinearLayoutManager.VERTICAL,false)
      ...
      //  val prevDaysInMonth = prevYearMonth.lengthOfMonth()
        ...
        for (i in 1..42) {
            if (i > dayOfWeek || i< daysInMonth - dayOfWeek)
                daysInMonthArray?.add(LocalDate.of(selectdate.dayOfYear, selectdate.dayOfMonth,i-dayOfWeek))
         *//*   else if (i > daysInMonth + dayOfWeek)
                daysInMonthArray.add(LocalDate.of(nextMonth.year, nextMonth.month, i - dayOfWeek - daysInMonth))
            if (i >= dayOfWeek || i< daysInMonth + dayOfWeek)
                daysInMonthArray?.add(LocalDate.of(selectdate.dayOfYear, selectdate.dayOfMonth,i-dayOfWeek))
            //LocalDate.of(selectdate.getYear(), selectdate.getMonth(), i - dayOfWeek)*//*
        }
        ...
        for (i in 1..42) {
            if (i <= dayOfWeek || i > daysInMonth + dayOfWeek)
                ...
                daysInMonthArray.add(LocalDate.of(
                   selectdate.getYear(),
                    selectdate.getMonth(),
                    i - dayOfWeek
                )
