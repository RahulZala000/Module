...
public class WeekView extends View {
    ...
    private int mHourSeparatorColor = Color.rgb(100, 100, 100);
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            float top = mHeaderHeight + mHeaderRowPadding * 2 + mCurrentOrigin.y + mHourHeight * (-2.5f) + mTimeTextHeight/2 + mHeaderMarginBottom;

            //canvas.drawLine(start,top,startPixel+mWidthPerDay,top,mHourSeparatorPaint);
        ...
        for (int dayNumber=leftDaysWithGaps+1; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays + 1; dayNumber++) {
            ...
            for (String line: dayLabel.split("\n")) {
                canvas.drawText(line, startPixel + mWidthPerDay / 2, y,  sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
                Paint mTextPaint=sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint;
                //y += mTextPaint.descent() - mTextPaint.ascent();
                y=y+mTextPaint.getTextSize();
            }
                ...
                public String interpretDate(Calendar date) {
                    try {
                        if (mNumberOfVisibleDays==1) {
                            ...
                            return date.getTime().getDate() + "\n" + new SimpleDateFormat("EEE").format(date.getTime()).toString(); //+ "\n" + new SimpleDateFormat("EEE").format(date.getTime()).toString();
                        }
                        //return String.format("%s\n%s", sdf.format(date.getTime()).toUpperCase(), localizedDate);
                    } catch (Exception e) {
                        e.printStackTrace();
                        return "";
                    }
